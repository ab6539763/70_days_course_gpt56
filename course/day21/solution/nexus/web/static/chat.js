...
function apiHeaders(extra = {}) {
  ...
  if (currentRevision !== null && currentRevision !== undefined) {
    headers["X-Expected-Revision"] = String(currentRevision);
  }
  return headers;
}

function handleRevisionConflict(error, data) {
  if (error && error.code === "REVISION_CONFLICT") {
    const actual = data && data.actual_revision;
    const expected = data && data.expected_revision;
    const hint = actual !== undefined
      ? `（期望 ${expected ?? currentRevision}，实际 ${actual}）`
      : "";
    setFeedback(`会话已被其他标签页更新，已为你刷新${hint}`, true);
    return true;
  }
  return false;
}
...
async function loadHealth() {
  ...
  const resumeHint = data.pending_resume_count
    ...
    : "";
  statusEl.textContent = `session=${currentSessionId?.slice(0, 12) || ""}…｜rev=${currentRevision}｜sessions=${data.session_count ?? "?"}${resumeHint}｜trace=${data.trace_id || ""}`;
}
...
async function loadMessages() {
  ...
  const data = await response.json();
  if (!data.ok) {
    if (handleRevisionConflict(data.error, data)) {
      await loadMessages();
      return;
    }
...
async function streamChat({ prompt = null, resumeToken = null } = {}) {
  ...
  const body = resumeToken
    ? { resume_token: resumeToken, expected_revision: currentRevision }
    : { prompt, expected_revision: currentRevision };
  ...
  while (true) {
    ...
    parsed.events.forEach((item) => {
      if (item.event === "resume" && item.data.partial) {
        ...
        if (handleRevisionConflict(item.data.error, item.data)) {
          throw new Error("REVISION_CONFLICT_HANDLED");
        }
  ...
  if (donePayload && typeof donePayload.revision === "number") {
    currentRevision = donePayload.revision;
  }
  if (interruptedPayload && typeof interruptedPayload.revision === "number") {
    currentRevision = interruptedPayload.revision;
  }
...
async function clearHistory() {
  ...
  const response = await fetch("/api/clear", {
    method: "POST",
    headers: apiHeaders({ "Content-Type": "application/json" }),
    body: JSON.stringify({ keep_system: true, expected_revision: currentRevision }),
  });
  const data = await response.json();
...
chatForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (String(error) === "REVISION_CONFLICT_HANDLED") {
      await loadMessages();
      return;
    }
    sendBtn.disabled = false;
...
ensureSession()
  .then(async () => {
    const rev = await fetch("/api/revision", { headers: apiHeaders() });
    const revData = await rev.json();
    if (revData.ok && typeof revData.revision === "number") {
      currentRevision = revData.revision;
    }
