...
ERROR_INVALID_RESUME = "INVALID_RESUME"
ERROR_RESUME_EXPIRED = "RESUME_EXPIRED"
ERROR_REVISION_CONFLICT = "REVISION_CONFLICT"
ERROR_REVISION_REQUIRED = "REVISION_REQUIRED"
...
ERROR_HTTP_STATUS = {
    ...
    ERROR_INVALID_RESUME: 400,
    ERROR_RESUME_EXPIRED: 410,
    ERROR_REVISION_CONFLICT: 409,
    ERROR_REVISION_REQUIRED: 400,
...
def revision_conflict_body(error) -> Dict[str, Any]:
    """构造 revision 冲突响应体。"""
    extra = {}
    if getattr(error, "expected", None) is not None:
        extra["expected_revision"] = error.expected
    if getattr(error, "actual", None) is not None:
        extra["actual_revision"] = error.actual
    return build_error_body(ERROR_REVISION_CONFLICT, error.message, **extra)
