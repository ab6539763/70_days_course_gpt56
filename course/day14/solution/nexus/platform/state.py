...
from nexus.utils.generators import iter_message_dicts
...
class PlatformState:
    ...
    def build_conversation_request(self, system_prompt="你是企业助手"):
        """把平台 messages 转为 API 请求 messages（含 system 注入）。"""
        history = list(iter_message_dicts(self.messages))
        messages = []
        has_system = any(item["role"] == "system" for item in history)
    ...
    def conversation_turn(
        ...
        """多轮对话单轮：持久化 user + assistant，并携带完整 history 调模型。"""
        self.add_message("user", user_prompt)
        model = self.active_model()
        request_messages = self.build_conversation_request(system_prompt)
