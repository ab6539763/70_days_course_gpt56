...
async function sendPrompt(prompt) {
  ...
  renderMessages(data.messages);
  const windowHint = data.window && data.window.window_applied
    ? `｜窗口 ${data.window.non_system_sent}/${data.window.non_system_total} 条`
    : "";
  setFeedback(`已保存 revision=${data.revision}${windowHint}`);
  await loadHealth();
}
