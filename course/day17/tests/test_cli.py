...
with tempfile.TemporaryDirectory(prefix="nexus-day17-cli-") as temporary:
    working = Path(temporary)
    (working / ".env").write_text(
        "NEXUS_API_KEY=cli-dotenv-key\nNEXUS_USE_MOCK=1\nNEXUS_WEB_PORT=18082\n",
        encoding="utf-8",
    )
    ...
    assert "Token 窗口" in first.stdout
    ...
    try:
        for _ in range(30):
            try:
                with urlopen("http://127.0.0.1:18082/api/window", timeout=1) as response:
                    payload = response.read().decode("utf-8")
                assert "0.0.17" not in payload or "default_history_window" in payload
                assert "history_window" in payload or "default_history_window" in payload
                break
            except Exception:
                time.sleep(0.2)
        else:
            raise AssertionError("/api/window 未在预期时间内可用")
    finally:
        web_proc.terminate()
        web_proc.wait(timeout=5)

server.shutdown()
print("Day 17 CLI/Web 验收通过：菜单17、/api/window 与 --web 均正确。")
