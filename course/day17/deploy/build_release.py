"""构建并验证 NexusAI 0.0.17 Token 窗口平台发布包。"""
...
SOURCE = ROOT / "course/day17/solution"
...
parser.add_argument("--output", type=Path, default=ROOT / "artifacts/day17")
...
web_smoke = subprocess.run(
    [
        ...
        (
            ...
            "assert c.post('/api/chat', json={'prompt':'x','max_history':4}).get_json()['ok']"
        ),
    ],
    cwd=stage,
    env=env,
