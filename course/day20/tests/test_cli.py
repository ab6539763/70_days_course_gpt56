...
with tempfile.TemporaryDirectory(prefix="nexus-day20-cli-") as temporary:
    working = Path(temporary)
    (working / ".env").write_text(
        "NEXUS_API_KEY=cli-dotenv-key\nNEXUS_USE_MOCK=1\nNEXUS_WEB_PORT=18085\n",
        encoding="utf-8",
    )
    ...
    assert "续传完成" in first.stdout or "resume_token" in first.stdout
    assert "会话隔离" in first.stdout
    ...
    try:
        for _ in range(30):
            try:
                req = Request(
                    "http://127.0.0.1:18085/api/session",
                    data=b"{}",
            ...
            raise AssertionError("POST /api/session 未在预期时间内可用")
    finally:
        web_proc.terminate()
        web_proc.wait(timeout=5)

server.shutdown()
print("Day 20 CLI/Web 验收通过：菜单20、断流恢复与 --web 均正确。")
