...
class BaseModel:
    ...
    def iter_stream_deltas(
        ...
        if force_mock or use_mock_mode():
            yield from self.simulate_stream(
                messages,
        ...
        for chunk in post_stream_json(
            ...
            if delta:
                if resume_from and not delta and emitted == 0:
                    continue
                emitted += 1
                if interrupt_after is not None and emitted > interrupt_after:
                    raise ApiCallError("流式上游断流（模拟）")
                yield delta
    ...
    def simulate_stream(
        ...
        """Mock 流式：把 simulate_response 按固定长度切片，支持断流与续传。"""
        text = self.simulate_response(messages)
        if resume_from:
            if text.startswith(resume_from):
                text = text[len(resume_from) :]
            else:
                text = text
        if chunk_size <= 0:
            chunk_size = DEFAULT_STREAM_CHUNK_SIZE
        ...
        for index in range(0, len(text), chunk_size):
            chunk_index += 1
            if interrupt_after is not None and chunk_index > interrupt_after:
                raise ApiCallError("流式 Mock 断流（模拟）")
