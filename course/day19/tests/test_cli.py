...
with tempfile.TemporaryDirectory(prefix="nexus-day19-cli-") as temporary:
    working = Path(temporary)
    (working / ".env").write_text(
        "NEXUS_API_KEY=cli-dotenv-key\nNEXUS_USE_MOCK=1\nNEXUS_WEB_PORT=18084\n",
        encoding="utf-8",
    )
    ...
    try:
        for _ in range(30):
            try:
                req = Request(
                    "http://127.0.0.1:18084/api/session",
                    data=b"{}",
...
server.shutdown()
print("Day 19 CLI/Web 验收通过：菜单19、POST /api/session 与 --web 均正确。")
