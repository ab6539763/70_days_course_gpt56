...
这一工作坊要求学员不写新代码，先解释为什么选择某个结构。每组扮演产品、开发、测试、安全和运维，针对同一业务需求给出数据结构、操作成本、失败模式和演化计划。目标不是找到永远正确的容器，而是让设计与访问模式、数据生命周期和风险相匹配。

### 21.1 场景一：为什么不是五个平行列表

一个看似可行的方案：

```python
task_ids = ["T1", "T2"]
titles = ["设计 Agent", "评审 RAG"]
priorities = [1, 2]
statuses = [False, True]
tags = [("agent",), ("rag",)]
```

同一个索引代表同一任务。问题是删除和排序时五个列表必须完全同步。若只对 priorities 排序，字段关系立即错乱；删除 task_ids[0] 却忘记 titles[0]，T2 会显示 T1 标题。当前嵌套列表把同一任务字段放在一个记录内，降低跨列表错位风险。

评审结论：平行列表适合教学某些向量化概念，但不适合当前 CRUD 模型。嵌套记录更接近后续字典、对象和数据库行。

### 21.2 场景二：为什么不只用集合保存任务

集合天然去重，似乎可保存任务元组：

```python
tasks = {
    (1, "T1", "设计 Agent", False, ("agent",)),
}
```

但任务完成需要把 False 改为 True，元组不可变，只能删除旧元组再加入新元组；集合也不提供产品需要的稳定业务顺序和切片。更重要的是，任务标题、状态变化后整个元组的哈希身份变化，不适合作主记录。

评审结论：集合保存稳定唯一键或标签非常合适，不承担可变任务主数据。
...
查看、排序和切片没有问题，但完成任务需创建新元组：

```python
old = tasks[index]
tasks[index] = (old[0], old[1], old[2], True, old[4])
```

这能工作，却在 Day 4 增加重复和位置错误。任务外层需要更新，使用列表更直观；标签创建后当前版本不编辑，用元组更合适。这说明同一业务对象内部可以组合不同结构。

### 21.4 场景四：为什么不是集合做展示

直接输出 `task_ids` 或 `all_tags` 的顺序可能随运行环境和数据变化，不应成为界面契约。用户截图、自动测试和审计对比都需要稳定结果。因此：

```python
print(sorted(all_tags))
```

集合负责唯一语义，排序列表负责展示语义。不要要求一个结构同时满足所有访问模式。

### 21.5 场景五：编号集合是否是缓存

`task_ids` 可以视为从 tasks 派生的内存索引。它不包含新业务事实，只让“是否重复”判断更直接。派生数据有两个策略：

1. 每次查询从主数据计算，简单一致但可能重复工作。
2. 保存派生索引，查询快但所有写操作必须同步。

当前选择第二种用于学习集合和一致性。批量清理后不尝试逐项修补，而是从主列表完整重建，降低遗漏风险。

### 21.6 场景六：如果新增中途失败

参考实现先完成所有输入校验和标签转换，再依次 append 与 add。若在 append 后、add 前发生不可预期异常，两个结构可能不一致。当前代码在这两行之间没有外部调用，风险很低，但并非原子事务。

数据库阶段会使用事务：要么任务行与唯一约束一起成功，要么全部回滚。今天的课程应明确单进程假设，不能把两行 Python 代码描述为强一致事务。

### 21.7 场景七：标签能否控制权限

用户可以输入任意标签，因此 `admin`、`finance` 只是文本，不能授予权限。可信权限必须来自经过认证和授权的系统数据。当前标签用于筛选和统计，不进入安全决策。

评审时把“业务分类标签”和“安全角色”分开。未来 Agent 工具调用若根据不可信标签放行，将形成权限提升漏洞。

### 21.8 场景八：优先级排序的隐含契约

`tasks.sort()` 依赖优先级在位置 0、编号在位置 1。若有人把模型改成 `[task_id, priority, ...]` 却忘记排序，代码仍运行但按编号排序，属于静默业务错误。字段表、排序测试和 Day 5 字典重构共同降低风险。

同优先级编号采用字符串顺序：`T10` 可能排在 `T2` 前。如果业务需要自然数字顺序，就必须解析编号数字部分或使用单独创建时间。当前验收没有这一要求，不暗中假设。

### 21.9 场景九：完成和删除的幂等性

完成同一任务两次，第二次不改变状态，属于幂等操作。删除同一任务两次，第二次返回未找到，主状态也不再变化。未来 API 的重试可能重复请求，幂等语义非常重要。

当前新增不是幂等：重复编号被拒绝而不是返回原任务。产品需要决定客户端重试时应把“已存在相同内容”视为成功还是冲突。Day 4 只实现明确冲突。

### 21.10 场景十：批量清理是删除还是归档

当前“清理已完成”永久从内存列表移除。生产环境通常需要保留审计、效果评估和恢复能力，可能改为归档状态或软删除。今天数据本就不持久化，清理只用于练习筛选与集合重建。

界面必须说“移除”，不能暗示记录已经安全归档。操作命名应与实际数据生命周期一致。

### 21.11 架构决策记录 ADR-004

```text
标题：Day 4 使用嵌套列表作为任务主数据，集合维护唯一编号
状态：已接受，计划 Day 5 替换
背景：学员已掌握流程控制，尚未学习字典和类
决策：
- tasks: list[list]，支持顺序、修改、排序、切片
- task_ids: set[str]，支持唯一成员检查
- tags: tuple[str]，表达标准化快照
后果：
- 可完成当前 CRUD 与统计
- 位置字段可读性差
- 两个结构有同步风险
...
试点员工完成并清理 T2 后，第二天想用 T2 重新录入修订任务。界面持续提示“任务编号已存在”，但查看列表没有 T2。客服认为数据丢失，开发认为数据重复，双方对系统事实理解不一致。
...
09:20 完成 T2
09:30 执行清理已完成
09:31 tasks 只剩 T1、T3
09:31 task_ids 仍为 {'T1','T2','T3'}
10:00 新增 T2 被拒绝
10:10 用户提交缺陷
```
...
预期最后新增成功，实际提示重复。最小复现让团队快速定位在清理路径，而不是怀疑排序或输入标准化。
...
```python
expected_ids = {task[1] for task in tasks}
```
...
新增前后、单条删除前后、批量清理前后都可检查。Day 10 使用断言或测试框架把它变成自动保护。

### 22.5 错误修复方案对比

方案 A：清理时逐个对完成任务调用 remove。  
风险：边遍历边改列表容易漏项，代码分支复杂。

方案 B：先构造保留任务，再从保留任务重建集合。  
优点：主数据结果清晰，派生索引完全重算，不依赖旧集合状态。
...
把“编号已存在”改成“编号可能已使用”不会修复状态。用户反馈更模糊，错误索引仍阻止业务。工程修复必须恢复数据不变量，不是掩盖症状。

### 22.7 为什么重启看似修复

退出后 tasks 和 task_ids 都丢失，重新运行可新增 T2。这可能让缺陷“无法复现”，但也说明系统没有持久化。测试必须在同一会话完成操作组合，不能每个步骤重启程序。
...
- 修改标签但标签全集未更新。
- 更改编号但旧编号仍在集合。
- 从 JSON 恢复 tasks 后忘记重建 task_ids。
- 多进程各自集合认为编号可用。

Day 5 加载 JSON 时，集合必须从加载后的任务重建，不能把旧进程集合写入文件当唯一事实。

---

## 23. 讲师分镜与课堂组织

### 23.1 09:00-09:20：从覆盖问题出发

【讲师】

“请大家打开 Day 3，找出只能保存一条任务的原因。不是因为 Python 做不到，而是变量每次只指向当前值。若再输入一条，前一条没有容器承载。今天先不讨论数据库，用内存结构解决多记录问题。”
...
提问：显示顺序、任务唯一性和状态修改分别需要什么特性？

### 23.2 09:20-10:00：数据建模评审

给每组五张字段卡，让学员排列位置。要求说明：
...
讲师暂不宣布答案，先用三个验收场景验证模型：排序、完成、标签去重。若模型无法自然支持，回到字段设计。

### 23.3 10:10-10:35：列表操作

现场从空列表开始，每次操作后打印：

```python
tasks = []
tasks.append([2, "T2", "评审", False, ()])
tasks.append([1, "T1", "设计", False, ()])
print(tasks)
tasks.sort()
print(tasks)
```

让学员预测第二次输出，不先运行。再修改 `tasks[0][3]`，观察嵌套可变性。

### 23.4 10:35-11:00：删除与切片

故意在循环中删除多个元素，展示跳项风险。然后比较：

```python
tasks = [task for task in tasks if not task[3]]
```

强调“保留谁”比“逐个删谁”在批量场景更易推理。切片实验覆盖 0、1、2、3、4 条任务。

### 23.5 11:00-11:30：集合与去重

让学员直接打印同一集合三次，说明即使某次看起来稳定，也不能把无序当接口保证。随后 `sorted`。
...
每一步在白板写中间结果。先去重再 lower 与先 lower 再去重对比。

### 23.6 11:30-12:00：元组与组合结构

尝试修改 `tags[0]`，观察 TypeError；再说明不可变只约束对象修改，不防止整个 task[4] 被替换。不要把元组描述为安全控制。
...
外层列表仍允许替换标签元组。数据结构表达意图，但业务权限仍需控制流和认证。

### 23.7 14:00-14:20：计划会
...
限制同时进行：先完成新增和查看闭环，再扩展修改路径。

### 23.8 14:20-15:20：结对实现新增与查看

驾驶员实现输入，领航员对照数据模型。十五分钟交换。每新增一个字段就打印 task 确认索引。
...
### 23.9 15:30-16:20：完成、删除、统计、清理

每加一个菜单操作就运行完整小场景。不要一次写完四个分支。
...
### 23.10 16:20-17:00：测试工作坊

把自动测试输入打印为时间线，学员标注每一行由哪个 `input()` 消费。交互测试最常见问题是输入错位：一次重试多消耗一行，后续所有输入进入错误字段。
...
### 23.11 17:00-17:30：发布部署

构建后不在 stage 目录直接宣布成功。测试再次解压 zip 到另一个目录，校验哈希并运行 CRUD。解释 stage 冒烟与解压后冒烟的差异：前者验证构建输入，后者验证真正归档内容。

### 23.12 19:00-20:00：独立作业

讲师只回答错误定位，不直接给完整代码。学员按“目标、输入、错误、最小复现、判断”模板提问。

### 23.13 20:00-21:00：代码评审与复盘

每组挑一处位置索引，说明它对应字段；挑一处集合更新，说明一致性；挑一条测试，说明业务风险。无法解释的代码即使运行也需要重写。
...
需求：保存按录入顺序展示的任务。答案：列表，因为有序且需修改。

需求：保存固定的三个试点部门。答案：元组可表达当前版本固定候选；集合也可快速成员检查，但展示顺序需额外处理。

需求：判断编号是否已存在。答案：集合表达唯一成员索引。

需求：保存任务内标签快照。答案：标准化后元组，避免原地改动；若需频繁编辑则列表或集合。
...
实际 tasks 有五个顶层元素，不是一条任务。修复 `tasks.append(task)`。

### 练习 3：位置更新

任务 `[2, "T2", "评审", False, ("rag",)]` 标记完成。
...
```python
high = [task for task in tasks if not task[3] and task[0] <= 2]
```

先写普通循环也正确，推导式只在表达仍清晰时使用。
...
```python
expected_ids = {task[1] for task in tasks}
print(task_ids == expected_ids)
```

结果 False 时不应盲目覆盖；先定位哪条写路径破坏状态，再修复并可重建。
...
共同标签 `a & b` 为 `{"rag"}`。展示用 `sorted(a & b)`。

### 练习 7：标签并集

`a | b` 得唯一的 rag、urgent、audit。顺序不保证。

### 练习 8：候选差集

```python
candidates = {"rag", "agent", "audit"}
used = {"rag"}
```

未用标签 `candidates - used` 为 agent、audit。
...
切片创建新的外层列表，但其中仍引用原任务子列表，所以状态修改会反映到 tasks。它是浅复制，不是完整独立副本。这一概念今天只观察，不深入对象复制实现。

### 练习 10：排序副作用

`tasks.sort()` 修改主列表；`sorted(tasks)` 返回新列表。若产品要求保留录入顺序并另显示优先级视图，应使用后者。
...
当前清理后允许复用，因为编号唯一范围仅为内存活跃任务。生产审计通常不允许历史编号复用，应由数据库全局唯一或带版本策略决定。测试必须与明确口径一致。

### 练习 13：浅复制风险

若希望预览只读，不要通过 `top_tasks` 修改子列表。接口层未来返回序列化副本，避免展示代码改主状态。

### 练习 14：空集合展示

`sorted(set())` 返回 `[]`，适合稳定展示；直接把空集合打印为 `set()` 也正确但界面格式不同。

### 练习 15：集合元素限制

`task_ids.add(["T1"])` 报 TypeError，因为列表可变不可哈希。字符串 `"T1"` 可加入。
...
外层遍历任务，内层遍历任务标签。空元组自然跳过。

### 练习 17：删除全部匹配

编号唯一时只删一个并 break。若数据已损坏有重复编号，当前删除只移除第一个，集合 remove 后剩余重复记录与索引不一致。这说明唯一约束应在写入时守住，并可增加数据完整性检查。

### 练习 18：稳定接口

测试不应断言完整集合字符串，而应断言排序后的结果。否则在不同 Python 运行中可能出现非业务失败。
