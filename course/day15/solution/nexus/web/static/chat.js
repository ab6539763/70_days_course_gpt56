...
async function loadHealth() {
  ...
  if (!data.ok) {
    statusEl.textContent = `服务异常：${data.error || response.status}`;
    return;
  }
...
async function loadMessages() {
  ...
  const data = await response.json();
  if (!data.ok) {
    setFeedback(data.error || "加载消息失败", true);
    return;
  }
...
async function sendPrompt(prompt) {
  ...
  if (!data.ok) {
    setFeedback(data.error || "发送失败", true);
    return;
  }
  renderMessages(data.messages);
...
async function clearHistory() {
  ...
  if (!data.ok) {
    setFeedback(data.error || "清空失败", true);
    return;
  }
  renderMessages(data.messages);
