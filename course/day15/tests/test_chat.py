"""Day 14 多轮对话、slash 命令与 history 单测。"""
...
changed1, _, reply1 = state.conversation_turn("第一轮问题")
assert changed1 and "HTTP mock 回复" in reply1
assert len(state.messages) == 2
assert state.messages[0].role == "user"
assert state.messages[1].role == "assistant"

changed2, _, reply2 = state.conversation_turn("第二轮追问")
assert changed2 and "HTTP mock 回复" in reply2
assert len(state.messages) == 4
request = state.build_conversation_request()
...
assert cleared and "已清空对话" in message
...
server.shutdown()
print("Day 14 多轮对话单测通过：history 持久化与 clear 均正确。")
