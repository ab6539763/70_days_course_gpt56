...
## 22. 企业案例推演：一条客服任务如何穿过治理链

这一节不增加新语法，而是把今天所有知识放进一次完整的企业评审。学员分成业务、产品、开发、测试和安全五个角色，所有结论必须引用数据或验收标准。

### 22.1 原始工单背景

客服专员张伟准备把以下模拟任务交给未来的 Agent：

```text
员工编号：" e 10086 "
部门："  客户服务中心 "
岗位：" ai support engineer "
任务："  Customer reports   ORDER-A100；请联系 DEMO-13800000000，
       并查询 order 状态。密码不要记录。 "
检索词："ORDER"
指定模拟敏感片段："DEMO-13800000000"
```

业务人员的目标是快速找到订单并生成回复草稿；安全人员的底线是模拟联系方式不能继续进入下游；产品人员希望列表能显示短预览；数据团队希望同一部门只有一个标准名称。

### 22.2 字段级决策

**员工编号**

它是标识符，不是自然语言。去掉首尾和内部半角空格、统一大写不会改变业务含义。输出 `E10086` 可与 Day 1 身份字段对齐。这里不能使用 `int`，因为字母前缀有业务意义。

**部门**

先折叠空白，再按已确认词表把“客户服务中心”替换为“客户服务部”。不能简单删除“中心”二字，因为其他组织名称可能真的包含“中心”；规则必须匹配完整已知别名。

**岗位**

试点采用 `title()` 得到 `Ai Support Engineer`。评审明确记录 AI 缩写显示不理想，但不在 Day 2 堆叠例外。Day 5 用标准词表把它映射为正式岗位。已知限制公开不等于忽视质量，而是把修复放到具备合适数据结构的迭代。

**任务描述**

`split()` 和 `" ".join(...)` 折叠连续空白；指定片段必须在任何预览、统计和输出之前替换；“密码”标签改为“凭据”，但这只是语义弱化，不代表发现了真实密码。最终结果只允许出现 `[已脱敏]`，不得出现模拟号码。

**检索副本**

创建小写副本做 `count` 和 `find`，展示文本不被覆盖。这样 `ORDER-A100` 与 `order` 都能命中，同时保留原任务的可读形式。关键词首次索引基于清洗后文本，测试报告必须说明，不能把它误解为原始文本位置。
...
2. 哪一步第一次形成标准化部门？
3. 脱敏发生在预览之前还是之后？
4. 是否有任何 `print` 使用 `raw_task_description`？
5. 检索为何不直接覆盖展示文本？
6. 如果敏感片段出现两次，规则是否全部替换？
7. 如果敏感片段为空，实际会怎样，哪个迭代修复？
8. 如果清洗后文本比原文更长，指标是否仍可解释？
9. 发布包是否包含测试输入或本地缓存？
10. 解压到无仓库文件的目录后能否启动？

每个回答都要指向代码、测试断言或需求条目。仅回答“应该没问题”不能通过评审。

### 22.4 故障注入演练

讲师依次制造四个故障，学员判断哪一层应发现：

**故障一：把脱敏语句移到预览之后。**  
代码仍能运行，普通输出可能也有掩码，但预览可能泄漏。功能端到端中的“不含原片段”断言应失败。这说明执行顺序属于安全属性。

**故障二：只对关键词调用 `lower()`。**  
大写正文不会匹配，预期命中 2 实际为 1。AC-05 和正常场景测试应失败。

**故障三：发布包混入 `__pycache__`。**  
功能可能正常，但发布文件集合断言失败。发布质量不仅是程序行为，还包括产物组成。

**故障四：手工修改 zip 内脚本。**  
若修改后未更新清单，SHA-256 校验失败；若同时手工更新清单，则说明简单哈希不能证明发布者身份，未来还需受控流水线和签名。学员应准确陈述保障边界。

### 22.5 业务验收会议记录

```text
会议：NexusAI 0.0.2 试点验收
需求：US-FOUNDATION-002
业务结论：部门别名、任务空白和订单检索满足试点口径
安全结论：指定模拟片段不进入结果；自动识别能力尚未具备
测试结论：正常、短文本、未命中、泄漏和发布链均需通过
运维结论：单文件 zip 可在 Python 3.10+ 独立目录启动
有条件项：空敏感片段必须在 Day 3 阻止
是否允许试点：仅允许模拟数据教学试点，不允许处理生产隐私
```
...
> Day 2 在 Day 1 员工字段前增加输入治理。程序按字段语义统一编号、组织、岗位和任务文本，在任何预览与输出之前精确替换用户指定的模拟敏感片段，并用小写副本完成检索。功能测试验证标准化、边界和不泄漏；发布测试验证构建、文件集合、哈希、解压和异目录启动。当前不具备空值校验和自动隐私识别，这两项分别进入 Day 3 与 Day 11。

如果复述中出现“已经完全解决敏感信息”“可以直接上生产”或“清洗率越高越好”，说明边界理解不合格，需要回到安全评审和指标解释重新学习。
