"""构建并验证 NexusAI 0.0.22 审计日志平台发布包。"""
...
(stage / "README.txt").write_text(
    "智枢 NexusAI 审计日志平台 0.0.22\nPython 3.10+\n"
    "依赖：pip install -r requirements.txt\n"
    "Web：PYTHONPATH=. python3 main.py --web\n"
    "审计：GET /api/audit 查询 revision 变更记录\n"
    "环境：NEXUS_AUDIT_DIR / NEXUS_AUDIT_MAX_ENTRIES\n",
    encoding="utf-8",
)
...
web_smoke = subprocess.run(
    [
        ...
        (
            ...
            "c.post('/api/chat', json={'prompt':'x','expected_revision':rev}, headers=h); "
            "audit=c.get('/api/audit', headers=h).get_json(); "
            "assert audit['count']>=2; "
            "assert 'web_chat' in {r['action'] for r in audit['records']}"
        ),
    ],
    cwd=stage,
    env=env,
