...
with tempfile.TemporaryDirectory(prefix="nexus-day22-release-") as temporary:
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        assert f"{NAME}/nexus/observability/audit_log.py" in names
    ...
    assert "审计日志" in cli.stdout
    assert "schema=4" in cli.stdout

server.shutdown()
print("Day 22 发布链验收通过：审计日志模块与部署恢复均正确。")
