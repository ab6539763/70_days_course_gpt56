...
class DocumentLoadError(NexusError):
    ...
    def __init__(self, message):
        super().__init__(message, code="DOCUMENT_LOAD")


class ApiCallError(NexusError):
    """大模型 HTTP API 调用或响应解析失败。"""

    def __init__(self, message):
        super().__init__(message, code="API_CALL")


class RevisionConflictError(NexusError):
    """乐观锁：expected_revision 与磁盘 revision 不一致。"""
