...
from nexus.observability.audit_log import resolve_audit_dir
...
def register_routes(app: Flask) -> None:
    ...
    audit_store = app.config.get("AUDIT_LOG_STORE")
    ...
    def health():
        ...
        audit_count = audit_store.total_count() if audit_store is not None else 0
        return api_ok(
            app_version=APP_VERSION,
            mock_mode=use_mock_mode(),
    ...
    def audit():
        ...
        try:
            ...
            return api_error(ERROR_VALIDATION, "limit 必须是整数")
        if limit <= 0 or limit > 500:
            return api_error(ERROR_VALIDATION, "limit 必须在 1-500 之间")
        try:
            records = manager.audit_log(limit=limit)
