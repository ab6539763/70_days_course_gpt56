...
def run_cli(data_file=DATA_FILE):
    bind_trace_id()
    print("=" * 76)
    print(f"智枢 NexusAI {APP_VERSION}｜Phase2 审计日志与 revision 变更追溯")
    ...
    while True:
        print(
            ...
            "12API对话 13配置 14多轮助手 15Web工作台 16API契约 17Token窗口 18SSE流式 19会话隔离 20断流恢复 21乐观锁 22审计日志 0退出"
        )
        ...
        try:
            if choice == "1":
                ...
                session_dir = ensure_session_dir(resolve_session_dir())
                registry = SessionRegistry(session_dir=session_dir)
                created = registry.create_session(owner="CLI_LOCK")
                session_id = created["session_id"]
                data_file = created["data_file"]
                writer_a = WebStateManager(data_file=data_file, session_id=session_id)
                writer_b = WebStateManager(data_file=data_file, session_id=session_id)
                print(
                    f"乐观锁演示：session={session_id[:20]}…｜"
                    f"expected_revision + persist_change 写前比对"
                )
                if not use_mock_mode():
                    print("提示：设置 NEXUS_USE_MOCK=1 可在 CLI 本地演示")
                else:
                    base_rev = writer_a.revision_info()["revision"]
                    writer_a.chat("甲标签写入", expected_revision=base_rev)
                    after_a = writer_a.revision_info()["revision"]
                    print(f"甲写入成功：revision {base_rev} → {after_a}")
                    try:
                        writer_b.chat("乙过期写入", expected_revision=base_rev)
                        print("未触发冲突（异常）")
                    except RevisionConflictError as error:
                        print(
                            f"捕获 REVISION_CONFLICT：期望 {error.expected}，"
                            f"实际 {error.actual}"
                        )
                    writer_b.chat("乙重试写入", expected_revision=after_a)
                    final_rev = writer_b.revision_info()["revision"]
                    print(f"乙重试成功：revision → {final_rev}")
            elif choice == "22":
                ...
                registry = SessionRegistry(session_dir=session_dir, audit_store=audit_store)
                created = registry.create_session(owner="CLI_AUDIT")
                session_id = created["session_id"]
                data_file = created["data_file"]
                manager = WebStateManager(
                    data_file=data_file,
                    session_id=session_id,
                    audit_store=audit_store,
                )
                print(
                    f"审计日志演示：session={session_id[:20]}…｜"
                    f"GET /api/audit + JSONL 落盘"
                )
                if not use_mock_mode():
                    print("提示：设置 NEXUS_USE_MOCK=1 可在 CLI 本地演示")
                else:
                    base_rev = manager.revision_info()["revision"]
                    manager.chat("审计演示对话", expected_revision=base_rev)
                    records = manager.audit_log(limit=10)
                    print(f"审计记录数：{len(records)}")
                    for item in records[:5]:
                        print(
                            f"  rev={item['revision']}｜action={item['action']}｜"
                            f"operator={item['operator']}｜trace={item['trace_id'][:12]}…"
                        )
                    print(f"审计目录：{audit_store.audit_dir}")
            elif choice == "0":
