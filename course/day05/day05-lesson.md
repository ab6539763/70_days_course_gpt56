...
这一部分用于上午后半段和晚自习。每个实验都要求学员先写出“Python 内存值、JSON 文本、再次加载后的 Python 类型”三列，再运行确认。目标是避免把看起来相似的字符串、布尔、数字和 null 混在一起。
...
加载后仍是 str，不能自动变 int。任务优先级若误保存为 `"1"`，范围和排序语义会变化。数据契约必须规定类型。

### 实验 2：整数

Python `100` 编码为 JSON `100`，加载后通常为 int。没有引号。JSON number 不区分数据库中的各种整数宽度。

### 实验 3：小数

Python `20.5` 编码为 `20.5`，加载为 float。金额和精确小数不能草率依赖浮点，后续财务模块另定格式。

### 实验 4：布尔

Python `True` 编码为 JSON `true`，加载后恢复 True。若文件写 `"true"`，它是字符串，在 Python 中非空字符串的真值为真，但语义与 bool 不同，容易产生隐蔽错误。

### 实验 5：空值

Python None 编码为 null。顶层 employee 在首次身份前为 null，表示字段存在但尚无值。字段完全缺失则可能表示结构损坏或旧版本，二者不能随意混同。

### 实验 6：空列表

`[]` 表示已知任务集合为空，不是未知值。统计可安全得到0。`tasks: null` 不满足当前 schema。

### 实验 7：空对象

`{}` 是存在但没有键的对象。employee `{}` 不等于 null，也不满足恢复身份所需字段。

### 实验 8：嵌套数组

tags 是字符串数组；tasks 是对象数组。读取 `data["tasks"][0]["tags"][0]` 时，每层类型都必须符合预期。

### 实验 9：元组

Python tuple 经过 json.dumps 通常成为数组，loads 后是 list，不会自动恢复 tuple。这叫信息损失：JSON 没有 tuple 类型。若业务必须区分，需要额外类型标记或加载转换。

### 实验 10：集合

```python
json.dumps({"rag", "agent"})
```

默认抛 TypeError。集合既非 JSON 类型，也无稳定顺序。正确做法是按业务规则转排序列表：

```python
json.dumps(sorted({"rag", "agent"}))
```

### 实验 11：字典键

JSON 对象键必须是字符串。Python 整数键序列化后会成为字符串键，加载后不再是整数。业务契约统一使用字符串字段名。
...
```python
json.dumps({"部门": "客户服务部"})
json.dumps({"部门": "客户服务部"}, ensure_ascii=False)
```

两者解析语义一致，后者更适合人工评审。文件本身仍需 UTF-8。

### 实验 13：缩进

比较紧凑和 indent=2 的字节数、可读性与 diff。当前小文件选择可读，模型 API 网络传输通常使用紧凑 JSON。

### 实验 14：键排序

两个插入顺序不同但键值相同的字典，使用 sort_keys=True 后文本稳定。稳定文本降低测试偶发差异，不表示数组会自动排序。

### 实验 15：末尾换行

许多文本工具期望 POSIX 文本文件最后一行以换行结束。解析 JSON 不要求，但工程规范可要求。测试 `raw.endswith("\n")`。

### 实验 16：dumps 不写文件

`json.dumps(data)` 只返回字符串。如果未把字符串写入文件，退出后仍无持久化。变量名可以叫 json_text，但它仍在内存。

### 实验 17：dump 需要文件对象

`json.dump(data, file)` 将文本写入已打开文件。`file` 的模式必须允许写，编码明确 UTF-8。

### 实验 18：loads 需要字符串

把 Path 或文件对象直接传 loads 不符合预期。可 read_text 后 loads，或使用 load(file)。

### 实验 19：load 读取文件

`json.load(file)` 从文件对象解析。空文件会 JSONDecodeError，因此参考实现先 read/strip 判断。
...
多数解析器保留最后一个值，但重复键会制造歧义，不应产生。由 json.dump 从字典生成可避免同一对象重复键。
...
不是标准 JSON。Python 字典允许代码中的尾随逗号，不代表 JSON 文本允许。不要混淆两种语法。

### 实验 22：注释

标准 JSON 不支持 `//` 或 `#` 注释。说明写 README 或使用明确配置格式。不要手动在业务数据文件插注释。

### 实验 23：NaN 与 Infinity

它们不是标准 JSON 数据互操作的安全选择。Python json 有特定行为，但企业契约应拒绝非有限数，避免不同语言解析不一致。当前优先级和 revision 都是有限整数。

### 实验 24：字符串转义

引号、反斜线和换行由 json 模块转义。手工拼接标题：

```python
'{"title": "' + title + '"}'
```

遇到引号会生成损坏或注入结构。必须使用序列化器。
...
对 JSON 支持类型，值结构应等价。tuple/set 等非原生语义不保证。

### 实验 26：深浅复制不是持久化

`backup = data` 只是另一个引用；修改 data 会影响 backup。JSON 往返可生成独立结构，但不应被滥用为高性能复制方案。

### 实验 27：任务字段缺失

顶层校验通过不代表每条任务完整。若 task 缺 priority，查看时 KeyError。记录为浅校验边界，后续模型验证。

### 实验 28：额外字段

当前程序会保留顶层和任务中的未知字段，只要完整文档 dump。是否允许未知字段需要 schema 策略；未来严格模型可能拒绝。

### 实验 29：键拼写

`task["is_dnoe"] = True` 会新增错误键，原 is_done 仍 False。字典具名提高可读性，但不提供编译期拼写检查。

### 实验 30：数据文件路径

从两个目录运行同一脚本，会产生两个 nexus_tasks.json。持久化是否“丢失”先核对工作目录，不立刻重建数据。
...
## 23. 模拟大模型 API JSON 解析工作坊

Day 12 才正式发起网络请求，但 Day 5 先理解返回结构。所有数据均为本地模拟，不包含真实密钥或调用结果。
...
1. response 字典取 choices。
2. choices 是列表，取第0项。
3. 选择项字典取 message。
4. message 字典取 content。

任何层类型或键变化都会影响读取。不能只记一条长表达式。
...
这只是当前响应验证，实际供应商可能有缓存、推理等细分字段，需按官方契约。

### 23.4 多 choices

若 choices 有多项，不能永远假设只看第0项。未来用循环处理。当前模型 API 常见 n=1，但 schema 是数组说明可能多条。

### 23.5 finish_reason

`stop` 可能表示自然结束，`length` 可能表示达到长度。应用不能只取 content 而忽略完成原因。Day 16 结合 max_tokens。

### 23.6 role

message 中 role 与 content 分开，为多轮 messages 做准备。不要把角色拼进 content 再丢失结构。
...
```json
{
  "error": {
    ...
    "message": "模拟错误，不含真实请求信息"
  }
}
```

读取 choices 前需要判断 error。条件控制已在 Day 3 学过，异常和 HTTP 状态 Day 10/12 完善。

### 23.8 Function Calling 预告

未来工具调用 arguments 常是 JSON 字符串嵌在外层 JSON：
...
外层 loads 后 arguments 仍是 str，需要再次 loads。不能看到花括号就假设已经是 dict。

### 23.9 JSON 输出约束

模型生成“看起来像 JSON”的文本可能有 Markdown 代码围栏、尾随逗号或字段缺失。生产应用必须结构化输出与校验，不用字符串切片猜测。Day 18 实现。

### 23.10 API 数据安全

真实响应可能含请求内容、用户文本、模型标识和用量。日志、课件和 Git 只使用模拟数据。API Key 永远不是 JSON 示例字段。
...
未来可以把模型建议保存为任务字段，但不能直接覆盖用户标题。建议数据应有来源、模型、时间和确认状态。今天只解析，不新增字段，避免超范围。
...
正常首次状态，不应报错。默认文档进入身份流程。

### 故障 2：文件为空

可能来自预创建挂载或上次写入中断。当前视为首次会覆盖空文件。生产环境应先调查来源并备份。

### 故障 3：JSON 少右括号

当前 JSONDecodeError，程序退出，不应自动覆盖。Day 10 捕获后提示备份/修复。

### 故障 4：schema_version 99

可解析但不理解，exit3，保持原字节。比语法损坏更容易给明确版本提示。

### 故障 5：缺 revision

exit3。不能假定0，否则会掩盖不完整文档。

### 故障 6：tasks 为对象

顶层键存在但类型错误，exit3。

### 故障 7：employee 缺 department

当前浅校验后在恢复访问时 KeyError。这是已知边界，后续逐字段模型。

### 故障 8：任务 priority 为字符串

装饰排序可能产生错误或错误顺序。测试 schema 应覆盖类型，后续实现。

### 故障 9：只写一半进程终止

直接 w 可能破坏原文件。Day 11 策略：写临时文件、flush、校验、原子 replace、保留备份。

### 故障 10：两个进程同时修改

两者读 revision4，一个新增写5，另一个删除也写5，前者更新丢失。revision 只观察，当前不检测冲突。数据库阶段使用事务/乐观锁。

### 故障 11：磁盘满

写失败可能抛 OSError。Day 10 捕获，Day 11 保证原文件不被提前截断。

### 故障 12：无写权限

打开文件失败。不能告诉用户“保存成功”。错误路径要保留内存状态还是退出，需要产品决策。

### 故障 13：人工编辑标签为重复

加载后程序不自动规范历史标签，统计集合会去重但文件仍重复。加载时迁移或验证以后实现。

### 故障 14：人工删除文件

下一次被视为首次，用户可能误以为身份消失。生产需备份、权限和审计。

### 故障 15：制品含 JSON

新环境直接恢复构建者身份，严重数据隔离错误。构建和发布测试双重负向断言。

### 故障 16：README 与脚本文件名不一致

部署用户无法启动。发布测试按实际脚本执行，README 仍需人工/自动内容检查。

### 故障 17：哈希清单包含数据文件

动态数据每次变化导致完整性失败，也暗示数据被当制品。清单只覆盖静态脚本。

### 故障 18：回滚旧程序读取新 schema

旧程序应拒绝未知版本，不应覆盖。回滚前必须评估数据版本兼容。

### 故障 19：revision 手工减少

当前程序接受，只会继续增加。生产可检测单调性需外部历史或数据库，单文件无法知道过去。

### 故障 20：文件被替换为符号链接

涉及文件系统安全，Day 11/部署环境讨论。当前教学版不处理。
...
- is_done false → status active。
- is_done true → status done。
- owner 取 employee_id。
- created_at 无历史事实，设 null，不伪造。
- schema_version 改2。
- revision 是否增加由迁移契约明确；建议记录一次迁移变更。
...
保留 v1 原文件。若 v2 验证失败，不覆盖 v1。若生产已在 v2 写入新字段，再回滚 v1 可能丢失信息，需要逆迁移或停止回滚。版本演化必须先设计兼容窗口。

### 25.6 为什么今天不实现

尚未学习函数、异常与可靠文件替换。桌面演练建立需求，不用无法解释的复杂脚本假装完成生产迁移。
