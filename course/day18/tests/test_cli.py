...
with tempfile.TemporaryDirectory(prefix="nexus-day18-cli-") as temporary:
    working = Path(temporary)
    (working / ".env").write_text(
        "NEXUS_API_KEY=cli-dotenv-key\nNEXUS_USE_MOCK=1\nNEXUS_WEB_PORT=18083\n",
        encoding="utf-8",
    )
    ...
    assert "SSE 流式" in first.stdout
    assert "chunk:" in first.stdout
    ...
    try:
        for _ in range(30):
            try:
                with urlopen(
                    "http://127.0.0.1:18083/api/openapi.json",
                    timeout=1,
                ) as response:
                    payload = response.read().decode("utf-8")
                assert "/api/chat/stream" in payload
                break
            except Exception:
                time.sleep(0.2)
        else:
            raise AssertionError("/api/openapi.json 未在预期时间内可用")
    finally:
        web_proc.terminate()
        web_proc.wait(timeout=5)

server.shutdown()
print("Day 18 CLI/Web 验收通过：菜单18、/api/chat/stream 与 --web 均正确。")
